...
By default, the `ard_tabulate()` function returns the statistics `"n"`,
`"N"`, and `"p"`, where little `"n"` are the counts for the variable
levels, and big `"N"` is the number of non-missing observations. The
