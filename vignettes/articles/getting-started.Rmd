...
To get a continuous variable summary, we will use the `ard_summary()` function from the **{cards}** package.
...
To get the categorical variable summary, we will use the `ard_tabulate()` function.
...
To get the dichotomous variable summary, we will use `ard_tabulate(value)`.
...
ard_stack(
  ...
  arrd_tabulate(variables = SEX, value = list(SEX = "F"))
)
```
