...
Next, we will review an examples for creating a basic adverse events (AE) table.
...
dat_ae <- tfrmt::data_ae |> 
  ...
  dplyr::filter(!param=="pval") |>
  dplyr::mutate(col1 = ifelse(col1=="n_pct", "n (%)",
                              ifelse(col1=="AEs", "[AEs]",
                                     col1))) 
...
tfrmt::print_to_gt(tfrmt_ae, dat_ae) |>
  gt::tab_options(
      table.font.size = 12 
  )|> 
...
The table above contains two different types of summaries: event-level (i.e. total number of AEs, which are displayed in the column labeled "[AEs]") and participant-level (i.e. number and percent of subjects with at least one AE, which are displayed in the column labeled "n (%)"). 
...
To get the participant level summaries, we will take a multi-step approach by calculating number and percent of subjects with: (1) at least one AE, (2) at least one AE by system organ class, and (3) at least one AE by system organ class and preferred term.

Because `ADAE` potentially contains multiple records per subject for a given AE, we must collapse the data so that it is subject-level. For the first step, we must reduce the data down to 1 record per subject with any AE (note that your data may contain derived flags to assist with this step):
```{r}
adae_subj <- ADAE |> dplyr::select(USUBJID, TRTA) |> unique()
```
...
```{r}
adae_soc <- ADAE |> dplyr::select(USUBJID, TRTA, AEBODSYS) |> unique()
```
...
```{r}
adae_pt <- ADAE |> dplyr::select(USUBJID, TRTA, AEBODSYS, AETERM) |> unique()
```
...
```{r}
ard_subj_all <- bind_ard(ard_subj_any, ard_subj_soc, ard_subj_pt)  
ard_subj_all
```
...
For event level summaries, we will be counting the total number of AEs and therefore can use the `ADAE` data directly. We will need to count AEs overall, by system organ class, and by preferred term (within system organ class).
...
```{r}
ard_event_all <- bind_ard(ard_event_any, ard_event_socpt)
ard_event_all
```
...
The subject and event level results can be combined using `bind_ard()` as such:

```{r}
bind_ard(ard_subj_all,
         ard_event_all|> dplyr::mutate(stat_name = "n_aes")) 
```
