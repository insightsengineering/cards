...
The goal of this article is to illustrate which {cards} functions are used to various types long data summaries: think ADAE, ADLB, ADCM, and other similarly structured data sets.

Generally, the solution to long data summaries lies with `ard_stack_hierarchical*()`, `tbl_strata()`, or even a call to a more basic function like `ard_tabulate()`. Herein, we will review these function and when each is needed.
...
gtsummary::tbl_hierarchical(
  ...
  gtsummary::modify_table_body(
    ~ .x |> 
      dplyr::filter(label %in% c("Number Subjects with AE",
...
In the table above, the AE _rate_ are reported for both the system organ class (SOC) and AE term.
That is, each AE is counted once per subject, and the rate each AE is then calculated; this is then repeated for SOC.
...
Let's inspect the different summary statistics returned.

**Adverse Event Rates**

The `ADAE` data frame is subset to remove duplicate AEs reported from each subject.
From there, the rates are calculated against the `ADSL` data frame passed in the `denominator` argument.
