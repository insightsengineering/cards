...
In this example we demonstrate how to use cards to perform a grouping with multiple variables and summary stats for categorical variables from long tables. Here, we define long tables as tables with multiple rows per subject (i.e. multiple records per subject).
...
We first use the `ard_categorical` function but note that all combinations of the AE term and body systems or organ class is provided whereas we would prefer to restrict this just to the groups that exist.
...
Using the hierarchical functionality in `cards` provides an ARD with all valid combinations regardless of whether these are observed.
...
Now we want to also include a group that gives the overall stats (i.e. dropping the TRTA grouping)
...
When using `ard_stack_hierachical` as we have specified below, the AEDECOD terms are not retained and therefore we can't map the stats to the AE terms.
...
Using no post-processing, the grouping cols are misaligned in the final ARD, i.e. group_1: AEBODSYS for the totals and group_1:TRTA for the grouped by treatment
